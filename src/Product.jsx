...
const Product = ({ product,cartArrHandle }) => {
    ...
    return (
        <div className='border-2 border-[#F2F2F2] p-6 rounded-2xl text-left space-y-4 relative'>
            ...
            <button onClick={handleCart} className='btn btn-ghost w-full rounded-full bg-linear-to-r from-[#4F39F6] to-[#9514FA] py-4 px-5 text-white font-bold'>{cart ? 'Buy Now' : 'Added To Cart'}</button>
        </div>
    );
};
